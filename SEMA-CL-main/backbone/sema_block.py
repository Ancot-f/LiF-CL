...
class SEMAModules(nn.Module):
    ...
    def add_adapter(self, initialize=False):
        ...
        new_adapter = AdapterModule(self.config, adapter_id, self.writer).to(self._device())
        self.newly_added = True
        self.added_for_task = True
    ...
    def forward(self, x):
        ...
        if not_addition_layer:
            ...
            if addition_criteria:
                self.add_adapter()
                out = {"func_out": torch.zeros_like(func_outs[0]), "rd_loss": torch.tensor(0., device=x.device), "added": True}
                return out
            else:
