...
class BaseLearner(object):
    ...
    def set_wandb_logger(self, wandb_logger):
        """Set the wandb logger (called by trainer)."""
        self._wandb_logger = wandb_logger

    def get_loss_tracker(self, task_id=None):
        """Get or create a LossTracker for the current task.

        The returned tracker accumulates per-batch losses and
        logs per-epoch averages to wandb automatically.
        """
        if task_id is None:
