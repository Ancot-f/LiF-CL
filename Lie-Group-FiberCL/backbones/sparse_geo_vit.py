"""
Sparse Geometry-Aware MoE Vision Transformer Backbone
======================================================

ViT-B/16 backbone with Sparse Group-MoE adapters in deep layers (9-11).

Layer layout:
  Layers 0-5:   Standard ViT (frozen, shared low-level vision)
  Layers 6-8:   Standard ViT (mid-level semantics)
  Layers 9-11:  Sparse Group-MoE active (sparse group selection,
                intra-group expert routing, Group-Aware AE/RD)

Deep layer output:
  u_l = ViTBlock_l(h_l)                  -- ViT self-attention output
  a_l = SparseGroupMoEAdapter_l(u_l)     -- sparse group MoE adapter
  h_{l+1} = u_l + MLP(u_l) + a_l         -- residual connection
"""
...
from backbones.sparse_geo_moe import SparseGroupMoEModules, SimpleAdapter
...
# ═══════════════════════════════════════════════════════════════════════════════
# Attention — Multi-head self-attention with separated Q/K/V
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# Block — ViT Transformer block with Sparse Group-MoE adapter
# ═══════════════════════════════════════════════════════════════════════════════

class Block(nn.Module):
    """ViT Block with Sparse Group-MoE adapter in deep layers.

    Shallow/Mid (0-8): Standard ViT (Attention + MLP)
    Deep (9-11):       Attention + MLP + SparseGroupMoEAdapter (parallel)
    """
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0.,
        ...
        if config.ffn_adapt:
            if is_deep:
                self.adapter_module = SparseGroupMoEModules(
                    config, layer_id=layer_id, writer=writer
                )
            else:
                # Shallow/Mid: simple SEMA-style bottleneck MLP (matches SEMA capacity)
    ...
    def forward(self, x, group_info=None):
        ...
        if self.is_deep:
            # Deep: MLP (frozen) + Sparse Group-MoE adapter (trainable) in parallel
            ...
            # Shallow/Mid: MLP (frozen) + SimpleAdapter (trainable) in parallel
            adapt_x = self.adapter(x)

            residual = x
            x = self.mlp_drop(self.act(self.fc1(self.norm2(x))))
            x = self.drop_path(self.mlp_drop(self.fc2(x)))
            x = residual + x + adapt_x
...
# ═══════════════════════════════════════════════════════════════════════════════
# VisionTransformer — Full Sparse Group-MoE ViT backbone
# ═══════════════════════════════════════════════════════════════════════════════

class VisionTransformer(nn.Module):
    """Sparse Group-MoE Vision Transformer backbone.

    Architecture:
      Input -> PatchEmbed -> PosEncoding -> Blocks[0..11] -> Pooling -> Features

    Blocks distribution:
      - Blocks 0-5:   Shallow (frozen, no adapter)
      - Blocks 6-8:   Mid (standard ViT)
      - Blocks 9-11:  Deep (Sparse Group-MoE + MambaFlow + Group-Aware AE/RD)

    Forward returns:
      - features:         [B, D] CLS token features
      - group_rd_loss:    scalar accumulated group RD loss
      - added_record:     [depth] expansion trigger record per layer
      - all_group_probs:  list of deep-layer group probabilities
      - all_z_scores:     list of deep-layer z-scores
      - all_selected_masks: list of selected group masks
    """
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3,
        ...
        print("I'm using ViT with Sparse Group-MoE adapters.")
        ...
        # Group-Structured Positional Routing (optional)
        self.use_group_pos = (
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            sm = blk_ret.get("selected_mask")
            if gp is not None:
                all_group_probs.append(gp.detach())
            if zs is not None:
                all_z_scores.append(zs.detach())
            if sm is not None:
                all_selected_masks.append(sm.detach())
...
def sparse_geo_vit_base_patch16_224(pretrained=True, tuning_config=None, **kwargs):
...
def sparse_geo_vit_base_patch16_224_in21k(pretrained=True, tuning_config=None, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12,
        mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
