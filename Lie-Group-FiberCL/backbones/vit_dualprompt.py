""" Vision Transformer (ViT) with DualPrompt for Continual Learning

基于 timm 库的 Vision Transformer 实现，增加了 DualPrompt 机制用于持续学习。
DualPrompt 方法提出两种互补的提示 (Prompt) 机制：

1. G-Prompt (通用提示 / General Prompt):
   - 与任务无关的全局提示，附加到所有输入上
   - 在指定的浅层 transformer 层中插入，捕获通用的视觉知识
   - 支持两种插入模式：
     (a) Prompt Tuning 模式: 直接将提示向量拼接到 token 序列中
     (b) Prefix Tuning 模式: 将提示作为 K/V 前缀拼接到注意力的 key 和 value 中

2. E-Prompt (专家提示 / Expert Prompt):
   - 与任务相关的专家提示，通过 L2P 风格的提示池机制动态选择
   - 在指定的深层 transformer 层中插入，捕获任务特定的知识
   - 同样支持 Prompt Tuning 和 Prefix Tuning 两种模式

核心思想：
  - G-Prompt 在早期层提供稳定的通用表示
  - E-Prompt 在后期层提供任务自适应能力
  - 二者协同工作，实现高效的持续学习而无需修改预训练模型参数

DualPrompt 论文: 'DualPrompt: Complementary Prompting for Rehearsal-free Continual Learning'
...
class PreT_Attention(nn.Module):
    """Prefix Tuning 多头自注意力模块

    在标准多头注意力的基础上，支持在 Key 和 Value 前面拼接提示向量 (prefix)。
    这是 Prefix Tuning 的核心实现：
      - 提示向量经过投影后，被拼接到 K 和 V 的前面
      - Q 保持不变，因此提示 token 可以影响注意力输出但不直接参与查询

    提示格式: prompt 形状为 (2, B, num_heads, prompt_length, head_dim)
      - prompt[0] 是 key_prefix，拼接到 K 前
      - prompt[1] 是 value_prefix，拼接到 V 前
    ...
    def forward(self, x, prompt):
        """Prefix Tuning 注意力前向传播

        Args:
            x: 输入张量，形状 (B, N, C)
            prompt: 提示张量，形状 (2, B, num_heads, prompt_length, head_dim)
                    prompt[0] 为 key 前缀，prompt[1] 为 value 前缀
                    None 表示不使用提示
        ...
        if prompt is not None:
            # 将 prefix prompt 拼接到 K 和 V 的前面
            # prompt 形状: (2, B, num_heads, prompt_length, head_dim)
            ...
            k = torch.cat([key_prefix, k], dim=2)   # 将 key 前缀拼接到 K 前
            v = torch.cat([value_prefix, v], dim=2) # 将 value 前缀拼接到 V 前
...
class Attention(nn.Module):
    ...
    用于不使用 Prefix Tuning 的层，以及 Prompt Tuning 模式下的层
    （Prompt Tuning 通过拼接 token 实现，无需特殊注意力模块）。
    """
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
    ...
    def forward(self, x, *args):
        ...
        Args:
            x: 输入张量，形状 (B, N, C)
            *args: 额外参数（用于兼容 PreT_Attention 的接口，不使用）
...
class LayerScale(nn.Module):
    ...
    对残差输出乘以一个可学习的缩放因子，用于稳定深层 transformer 训练。
    """
...
class Block(nn.Module):
    """Vision Transformer 的基本 Transformer 块

    支持可选的注意力层类型 (attn_layer):
      - Attention: 标准注意力（用于 Prompt Tuning 或不使用提示的层）
      - PreT_Attention: Prefix Tuning 注意力（将提示拼接到 K/V 前缀）

    前向传播时通过 prompt 参数传递提示信息：
      - Prefix Tuning 模式: prompt 作为 K/V 前缀传入注意力层
      - Prompt Tuning 模式: prompt 在 Block 外部拼接到 token 序列中
    """
    ...
    def forward(self, x, prompt=None):
        ...
        Args:
            x: token 序列，形状 (B, N, C)
            prompt: Prefix Tuning 的提示张量，用于拼接到 K/V 前缀；
                    None 或不传时按标准注意力处理
        """
...
class ResPostBlock(nn.Module):
    """残差后归一化 Transformer 块 (Residual Post-Norm Block)

    归一化放在注意力/MLP 之后，而非之前。
    """
    def __init__(
            self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., init_values=None,
...
class VisionTransformer(nn.Module):
    """ Vision Transformer with DualPrompt (G-Prompt + E-Prompt) for Continual Learning

    在标准 ViT 基础上增加了 DualPrompt 双提示机制：

    提示类型:
    1. G-Prompt (通用提示):
       - 全局共享的可学习提示
       - 在指定的浅层 (g_prompt_layer_idx) 中插入
       - 提供任务无关的通用视觉先验

    2. E-Prompt (专家提示):
       - 基于 L2P 机制的提示池，通过 query-key 相似度动态选择
       - 在指定的深层 (e_prompt_layer_idx) 中插入
       - 提供任务特定的自适应能力

    插入模式:
    - Prompt Tuning: 将提示作为额外的 token 拼接到输入序列中
    - Prefix Tuning: 将提示作为 K/V 前缀拼接到注意力层的 key 和 value 中
    ...
    def __init__(
        ...
        Args:
            ...
            embed_dim (int): 嵌入维度
            depth (int): transformer 深度（总层数）
            num_heads (int): 注意力头数
            mlp_ratio (int): MLP 隐藏层维度比值
            qkv_bias (bool): QKV 投影是否使用偏置
            init_values: (float): LayerScale 初始值
            class_token (bool): 是否使用 class token
            fc_norm (Optional[bool]): 分类头前的归一化
            ...
            act_layer: (nn.Module): MLP 激活函数层
            block_fn: (nn.Module): transformer 块类型
            prompt_pool (bool): 是否使用提示池
            prompt_length (int): E-Prompt 中每个提示的长度
            pool_size (int): E-Prompt 池中的提示总数量
            top_k (int): E-Prompt 每次选择的提示数量
            use_g_prompt (bool): 是否使用 G-Prompt（通用提示）
            g_prompt_length (int): G-Prompt 的长度
            g_prompt_layer_idx (list[int]): 插入 G-Prompt 的层索引列表
            use_prefix_tune_for_g_prompt (bool): G-Prompt 是否使用 Prefix Tuning
            use_e_prompt (bool): 是否使用 E-Prompt（专家提示）
            e_prompt_layer_idx (list[int]): 插入 E-Prompt 的层索引列表
            use_prefix_tune_for_e_prompt (bool): E-Prompt 是否使用 Prefix Tuning
            same_key_value (bool): Prefix Tuning 时 key 和 value 是否共享同一个提示
        """
        super().__init__()
        ...
        # num_g_prompt: 实际需要附加 G-Prompt 的层数
        # 在官方代码中，按总层数创建 G-Prompt 参数，再按索引选择实际使用的层
        ...
        # 如果两种提示都不使用 Prefix Tuning，则不需要 Prefix Tuning 注意力
        ...
        if use_g_prompt and g_prompt_length is not None and len(g_prompt_layer_idx) != 0:
            if not use_prefix_tune_for_g_prompt:
                # Prompt Tuning 模式: G-Prompt 形状为 (num_g_prompt, g_prompt_length, embed_dim)
                # 每层一个独立的提示张量
                ...
                # Prefix Tuning 模式: G-Prompt 形状包含 num_heads 和 head_dim 维度
                if same_key_value:
                    ...
                    if prompt_init == 'zero':
                        ...
                        self.g_prompt = nn.Parameter(torch.randn(g_prompt_shape))
                        nn.init.uniform_(self.g_prompt, -1, 1)
                    self.g_prompt = self.g_prompt.repeat(1, 2, 1, 1, 1)  # 复制为 key 和 value 两份
                else:
        ...
        if not (use_g_prompt or use_e_prompt):
            ...
            # 仅使用 Prompt Tuning（拼接 token），标准注意力即可
            attn_layer = Attention
        else:
            # 使用了 Prefix Tuning，需要能处理 K/V 前缀的特殊注意力
            attn_layer = PreT_Attention

        # ============ 计算提示的总长度（用于分类头定位 CLS token） ============
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        """提取图像特征（包含 DualPrompt 的前向逻辑）

        这是 DualPrompt 方法的核心实现。每一层根据其索引判断：
        - 是否在 g_prompt_layer_idx 中 --> 插入 G-Prompt
        - 是否在 e_prompt_layer_idx 中 --> 插入 E-Prompt
        - 否则按标准 transformer 层处理

        插入方式：
        - Prefix Tuning: 通过 block(x, prompt=g/e_prompt) 将提示传入 PreT_Attention
        - Prompt Tuning: 在调用 block 前将提示拼接到 x 的 token 维度

        执行顺序: CLS token 拼接 -> 位置嵌入 -> 逐层 transformer（含提示插入）-> 最终归一化

        Args:
            x: 输入图像张量，形状 (B, C, H, W)
            task_id: 任务 ID（用于 E-Prompt 的提示掩码）
            cls_features: 预计算的 CLS 特征
            train: 是否为训练模式

        Returns:
            dict: 包含 'x' 键（最终输出）的字典
        """
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                if self.use_prompt_mask and train:
                    ...
                    if end > self.e_prompt.pool_size:
                        prompt_mask = None  # 超出池范围时退化为无掩码模式
                else:
                    prompt_mask = None
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        ...
                        if self.use_prefix_tune_for_g_prompt:
                            g_prompt_counter += 1
                            # Prefix Tuning: [B, 2, g_prompt_length, num_heads, embed_dim // num_heads]
                            # 使用 index_select 获取当前层对应的 G-Prompt
                        ...
                        if self.use_prefix_tune_for_e_prompt:
                            # Prefix Tuning: [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                            x = block(x, prompt=e_prompt[e_prompt_counter])
                        else:
                            # Prompt Tuning: [B, top_k * e_prompt_length, embed_dim]
                            # 直接将提示 token 拼接到输入序列前面
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        根据 head_type 和 prompt_pool 配置确定如何提取分类特征。
        如果使用提示池，需要跳过提示 token 再提取 CLS token；
        如果不使用提示池，直接从索引 0 提取 CLS token。

        Args:
            res: forward_features 返回的字典
            pre_logits: 是否返回 logits 之前的特征

        Returns:
            dict: 包含 'pre_logits' 和 'logits' 的字典
        """
        x = res['x']
        if self.class_token and self.head_type == 'token':
            if self.prompt_pool:
                # 跳过的 token 数 = total_prompt_len，CLS token 在 total_prompt_len 位置
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False):
        ...
        Args:
            x: 输入图像张量
            task_id: 任务 ID
            cls_features: 预计算的 CLS 特征（可选）
            train: 是否为训练模式

        Returns:
            dict: 包含 'x', 'pre_logits', 'logits' 等键的字典
        """
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    从 Google JAX 实现的 .npz 格式检查点加载权重到 PyTorch 模型中
    """
    import numpy as np
...
def resize_pos_embed(posemb, posemb_new, num_prefix_tokens=1, gs_new=()):
    ...
    通过双三次插值缩放位置嵌入，并为提示 token 扩展前缀位置嵌入。
    """
    ...
    if ntok_new > gs_old ** 2:
        ntok_new -= gs_old ** 2
        # 扩展 CLS token 的位置嵌入以为提示 token 预留空间
...
# ============ 注册各种 ViT 变体模型（均带有 DualPrompt 支持） ============
