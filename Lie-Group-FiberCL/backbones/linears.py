...
提供多种分类器头实现，适应不同的增量学习策略：

  - SimpleLinear: 标准线性分类器，输出 {"logits": ...}
  - CosineLinear: 余弦相似度分类器（L2 归一化特征 + L2 归一化权重 + 缩放因子 sigma）
  - SplitCosineLinear: 拆分余弦分类器（分别处理旧类和新类）
  - SimpleContinualLinear: 持续线性分类器，每个任务添加独立头
...
class SimpleLinear(nn.Module):
    ...
    输出格式：{"logits": tensor}
    适用于简单微调 (finetune)、DER、FOSTER 等方法。
    """
...
class CosineLinear(nn.Module):
    """余弦相似度分类器 —— 基于特征和权重的 L2 归一化。

    特点：
      - 对输入特征和分类权重做 L2 归一化
      - 余弦相似度 = normalize(x) @ normalize(W).T
      - 可学习的温度参数 sigma 缩放输出
      - 支持代理数量 nb_proxy（每类多个代理向量）

    广泛用于 SimpleCIL、L2P、DualPrompt 等方法。
    """
...
class SimpleContinualLinear(nn.Module):
    ...
    特点：
      - 每个新任务添加一个新的 nn.Linear 头
      - 旧任务的头被冻结（防止遗忘）
      - 所有任务头的输出拼接为最终 logits
      - 支持 feat_expand（多特征输入）和 LayerNorm

    适用于 L2P、DualPrompt 等 Prompt 方法的分类器。
    """
