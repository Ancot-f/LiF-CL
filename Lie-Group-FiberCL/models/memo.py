"""
MEMO: Memory-Efficient MOdel for Class-Incremental Learning.

MEMO 通过分离的通用特征提取器 (TaskAgnosticExtractor) 和任务特定的
自适应模块 (AdaptiveExtractors) 实现记忆高效的持续学习.

架构:
  - TaskAgnosticExtractor: 任务无关的通用特征提取器, 可冻结或继续训练.
  - AdaptiveExtractors: 每任务一个自适应模块, 捕获任务特定特征.
  - aux_logits: 对每个 AdaptiveExtractor 输出进行辅助监督.

核心机制:
  - train_base: 控制通用模块是否在后续任务中继续训练.
  - train_adaptive: 控制旧的 AdaptiveExtractors 是否可训练.
  - skip: 跳过初始训练, 直接加载 checkpoint.
  - alpha_aux: 辅助损失权重.

参考论文: Zhou et al., MEMO (ECCV/CVPR).

核心特点:
  - 通用 + 自适应双分支架构.
  - 辅助分类损失 + 主分类损失联合优化.
  - 使用回放内存 (rehearsal memory).
  - Herding selection 构建样本回放集.
"""
...
class Learner(BaseLearner):
    """
    MEMO (Memory-Efficient MOdel) 学习者.

    通过 TaskAgnosticExtractor + AdaptiveExtractors 双分支架构
    实现记忆高效的持续学习. 通用模块负责共享知识,
    自适应模块捕获任务特定特征.

    关键属性:
      - _old_base: 旧的通用模块 (暂未用于蒸馏).
      - AdaptiveNet: 含通用提取器和自适应模块列表的网络.
    """
    def __init__(self, args):
        ...
        Args:
            args: 配置字典, 含 train_base, train_adaptive, skip 等.
        """
        super().__init__(args)
        self.args = args
        self._old_base = None
        # AdaptiveNet: 通用 + 自适应双分支网络
    ...
    def incremental_train(self, data_manager):
        ...
        扩展 FC 层, 设置 AdaptiveExtractors 训练状态, 训练后更新回放内存.
    ...
    def set_network(self):
        ...
        规则:
          - TaskAgnosticExtractor: 按 train_base 配置.
          - AdaptiveExtractors[-1] (最新): 总是训练.
          - AdaptiveExtractors[0..cur_task-1]: 按 train_adaptive 配置.
        """
        if len(self._multiple_gpus) > 1:
            self._network = self._network.module
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        核心损失 = loss_clf + alpha_aux * loss_aux:
          - loss_clf: 主分类器交叉熵 (所有类别).
          - loss_aux: 辅助分类器交叉熵 (旧类别 -> 0 背景, 新类别 -> 正整数).
            辅助分类为每个 AdaptiveExtractor 提供直接监督信号.
